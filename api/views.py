...
from .models import (CommonMachineSetting, UniqueMachineSetting, Machine,
                     ...
                     DoseComment, Dose,
                     DoctorViewStorage, MachineHardware, UserProfile)
from .serializers import (CommonMachineSettingSerializer, UniqueMachineSettingSerializer, MachineSerializer,
                          ...
                          DoseCommentSerializer, DoseSerializer,
                          DoctorViewStorageSerializer, MachineHardwareSerializer)
...
def get_all_cities(request):
    ...
    for machine in Machine.objects.all():
        try:
            ...
            print("Failed to retrieve location for", machine.machine_city, machine.machine_state,
                  machine.machine_country)
...
class DoseViewSet(viewsets.ModelViewSet):
    ...
    http_method_names = ['get', 'post', 'put', 'patch','delete']  # add put and patch here
    def perform_create(self, serializer):
        package_id = serializer.validated_data['package_id']
        patient_id = package_id.patient_id
        machine_id = patient_id.machine_id
        serializer.save(machine_id=machine_id, patient_id=patient_id)
...
# class MachineHardwareViewSet(viewsets.ModelViewSet):
#     queryset = MachineHardware.objects.all()
#     serializer_class = MachineHardwareSerializer
#
#     @action(detail=True, methods=['GET', 'PATCH'])
#     def last_machine_hardware(self, request, pk=None):
#         try:
#             machine_hardware = MachineHardware.objects.filter(machine_id=pk, status__in=['Not Started']).order_by('-created_at').first()
#         except MachineHardware.DoesNotExist:
#             return Response(status=status.HTTP_404_NOT_FOUND)
#
#         if request.method == 'GET':
#             serializer = MachineHardwareSerializer(machine_hardware)
#             return Response(serializer.data)
#         elif request.method == 'PATCH':
#             serializer = MachineHardwareSerializer(machine_hardware, data=request.data, partial=True)
...
#         machine_hardware = self.get_object()
#         serializer = MachineHardwareSerializer(machine_hardware, data=request.data, partial=True)
#         if serializer.is_valid():
#             serializer.save()
#             return Response(serializer.data)
    ...
    def last_machine_hardware(self, request, pk=None):
        ...
        if request.method == 'GET':
            ...
            if serializer.is_valid():
                ...
                # Update associated Dose object if status is provided in patch data
                dose_id = serializer.validated_data.get('dose_id')
                dose_status = serializer.validated_data.get('status')
                if dose_id and dose_status:
...
class DoctorViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if patient_history:
            ...
            medicine = Medicine.objects.filter(patienthistory=patient_history)
        ...
        if patient_history:
            patient_history_data = {
                ...
                # 'pdf':patient_history.pdf,
                'photo_of_date': [photo.photo.url for photo in patient_history_photos],
                'medicine':[medicine.name for medicine in medicine]
            }

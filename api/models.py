...
from .validators import validate_audio_file_extension, validate_image_file_extension, validate_pdf_file_extension
# Create your models here.
class UserProfile(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE)
    role = models.CharField(max_length=20, choices=(('admin', 'admin'), ('developer', 'developer'), ('operator', 'operator')),default='operater')
    def __str__(self):
...
class UniqueMachineSetting(models.Model):
    ...
    audio6_name=models.CharField(max_length=100, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class PatientHistoryPhotos(models.Model):
    ...
    photo = models.ImageField(upload_to='patient_history_photos', blank=True,validators=[validate_image_file_extension])
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class PatientHistory(models.Model):
    ...
    pdf=models.FileField(upload_to='pdfs', blank=True,validators=[validate_pdf_file_extension])
    ...
    rating=models.CharField(max_length=2000, blank=True)
...
class Patient(models.Model):
    ...
    photo = models.ImageField(upload_to='patient_photo', blank=True,validators=[validate_image_file_extension])
...
class Dose(models.Model):
    package_id = models.ForeignKey(Package, on_delete=models.CASCADE, blank=True)
...
#Machine Hardware
class MachineHardware(models.Model):
    machine_hardware_id=models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    machine_id=models.ForeignKey(Machine,on_delete=models.CASCADE,blank=True,null=True)
    patient_id=models.ForeignKey(Patient,on_delete=models.CASCADE,blank=True,null=True)
    full_name=models.CharField(max_length=50,default=0)
    dose_id=models.ForeignKey(Dose,on_delete=models.CASCADE,blank=True,null=True)
    j_cm2 = models.FloatField(blank=False,default=0)
    choices = (('Tube A', 'Tube A'), ('Tube B', 'Tube B'),(0,0))
    tube = models.CharField(max_length=200, blank=False, choices=choices)
    status = (
    ('Not Started', 'Not Started'), ('In Progress', 'In Progress'), ('Paused', 'Paused'), ('Completed', 'Completed'))
    status = models.CharField(max_length=200, choices=status, default='Not Started')
